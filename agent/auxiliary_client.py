...
# ── Async ACP Wrapper ──────────────────────────────────────────────────────
# CopilotACPClient is synchronous.  When auxiliary vision needs an async
# client (async_call_llm), we must wrap the sync client so that
# ``client.chat.completions.create()`` works in the async loop.
...
class _AsyncACPChat:
    """Thin async proxy that delegates to the sync CopilotACPClient."""

    def __init__(self, sync_client):
        self._sync = sync_client
        self.completions = self  # client.chat.completions.create() works

    async def create(self, **kwargs):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, lambda: self._sync.chat.completions.create(**kwargs)
        )


class _AsyncACPWrapper:
    """Minimal OpenAI-client-like wrapper exposing ``chat`` attribute."""

    def __init__(self, sync_client):
        self._sync = sync_client
        self.chat = types.SimpleNamespace(completions=_AsyncACPChat(sync_client))
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        ...
        if isinstance(sync_client, CopilotACPClient):
            # Wrap sync CopilotACPClient for async use via asyncio.to_thread.
            # Without this, async_call_llm (used by vision_analyze) fails with
            # "object types.SimpleNamespace can't be used in 'await' expression".
            return _AsyncACPWrapper(sync_client), model
    except ImportError:
        pass
