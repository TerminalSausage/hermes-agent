...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        # ── NachoTek patch: /note and /btw slash commands ──
        ...
        # ── NachoTek patch: /customer slash command (Ada profile only) ──
        ...
        if get_active_profile_name().lower() == "ada":
            ...
            @discord.app_commands.describe(
                ...
                action="Action: open, close, list (default: open)",
            )
            ...
            async def slash_customer(
                ...
                if not name or action.lower() == "list":
                    root = _customer_kb_root()
                    customers_dir = root / "Customers"
                    lines = ["**Customer Workspaces:**"]
                    if customers_dir.exists():
                        for d in sorted(customers_dir.iterdir()):
                            if d.is_dir():
                                lines.append(f"• {d.name}")
                    await interaction.response.send_message("\n".join(lines) or "No customers found.", ephemeral=True)
                    return
                await self._run_simple_slash(interaction, f"/customer {action} {name}")
